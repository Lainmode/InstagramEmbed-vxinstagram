...
namespace InstagramEmbed.Controllers
{
    public sealed class HomeController : Controller
    {
        ...
        private readonly InstaDownCacheService _instadown;
        ...
        public HomeController(
            ...
            InstaDownCacheService instadown,
            DonateMessageService donate,
            IHttpClientFactory factory,
            ILogger<HomeController> logger,
            IOptions<DonationSettings> options)
        {
            _posts = posts;
            _instadown = instadown;
            _donate = donate;
        ...
        private bool IsInstaDownHost =>
            Request.Host.Host.StartsWith("d.", StringComparison.OrdinalIgnoreCase) || Request.Host.Host.StartsWith("www.d.", StringComparison.OrdinalIgnoreCase);

        private Task<CachedPost?> GetPostAsync(string cacheId, string instagramUrl)
        {
            if (IsInstaDownHost)
            {
                ViewBag.IsInstaDownLink = true;
                return _instadown.GetOrFetchAsync(cacheId, instagramUrl);
            }

            else
            {
                return _posts.GetOrFetchAsync(cacheId, instagramUrl);
        ...
        [HttpGet]
        public IActionResult SetDonationVariables([FromQuery] string pw,
            [FromQuery] int current, [FromQuery] int? target)
        {
            if (pw != _settings.Password) return BadRequest();
        ...
        {
            _logger.LogInformation("HITs hit: ua={UA} host={Host}",
                Request.Headers.UserAgent.ToString(), Request.Host.Host);
            ...
            try
            {
                ...
                string cacheId = isStoriesNoId ? Uri.EscapeDataString(instagramUrl) : id;

                ViewBag.PostId = cacheId;
                ViewBag.MaybeDonate = _donate.MaybeGetDonateMessage();

                var post = await GetPostAsync(cacheId, instagramUrl);
        ...
        public async Task<IActionResult> OffloadPost(string id, int? order,
            ...
            [FromQuery(Name = "order")] int? orderQuery)
        {
            ...
            var post = await GetPostAsync(id, $"https://instagram.com/p/{id}/");
            if (post == null) return NotFound();
            ...
            if (isTelegramBot && entry.MediaType == "video")
            {
                try
                {
                    ...
                    var proxyResponse = await _http.SendAsync(proxyRequest,
                    ...
                    if (proxyResponse.IsSuccessStatusCode)
                    {
                        ...
                        if (proxyResponse.Content.Headers.ContentLength.HasValue)
                            HttpContext.Response.Headers["Content-Length"] =
                                proxyResponse.Content.Headers.ContentLength.Value.ToString();
                    ...
                    _logger.LogWarning("Telegram proxy got {Status} for {Url}",
                        proxyResponse.StatusCode, targetUrl);
                }
                catch (Exception ex)
                {
        ...
        public IActionResult OEmbed(string? username, string? desc,
            string? likescomments, string donateMessage)
        {
        ...
        public async Task<IActionResult> Activity(string contextBase64)
        {
            ...
            // ── route to InstaDown or snapsave based on host ──────────────────
            var post = await GetPostAsync(cacheId, igUrl);
            if (post == null) return NotFound();
            ...
            var orderedMedia = post.Media
                ...
                .Select((m, i) =>
                {
                    ...
                    return new MediaAttachment
                    {
                        id = cacheId,
                        type = m.MediaType,
                        url = $"{host}/offload/{Uri.EscapeDataString(cacheId)}?order={globalIdx}",
                        preview_url = $"{host}/offload/{Uri.EscapeDataString(cacheId)}?order={globalIdx}&thumbnail=true",
            ...
            CachedPost post, int orderIndex)
        {
            ...
            ViewBag.IsPhoto = media.MediaType == "image";
